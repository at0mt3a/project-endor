...
class PGWrapper {
  ...
  async shutdown() {
    ...
    while (this._pgQueue.length > 0)
      setTimeout(
        () => console.warn("Waiting for postgres to finish before closing..."),
        100
      );
    ...
    return new Promise((resolve, reject) => {
      this._pool
        ...
        .catch(err => {
          return reject(
            new Error(
              `PostgresWrapper Error: ${err}, Query: ${query} Params: ${params}`
            )
          );
        });
    });
  ...
  logError(errorParams) {
    ...
    console.error(
      `Failed to execute postgres ${method} query: ${query} Params: ${params}`,
      err
    );
